...
import os
from typing import List, Set, Dict, Tuple, Optional, Union, Sequence, Iterable
...
def bin_partition(
        ...
        return_edges: bool=False) -> np.ndarray:
    ...
    Args:
        ...
        binDict (dict, optional): TODO: implement passing binning parameters as dictionary or other methdods
        binAxes (list): List of names of the axes (columns) on which to calculate the histogram.
            The order will be the order of the dimensions in the resulting array.
        nBins (int or list, optional): List of number of points along the different axes. Defaults to None.
        binranges (tuple, optional): list of tuples containing the start and end point of the binning range. Defaults to None.
        hist_mode (str, optional): Histogram calculation method. Choose between "numpy" which uses numpy.histogramdd, and "numba"
            which uses a numba powered similar method. Defaults to 'numba'.
        jitterParams (dict, optional): Not yet Implemented. Defaults to None.
        return_edges: (bool, optional): If true, returns a list of D arrays describing the bin edges for each dimension, similar to 
            the behaviour of np.histogramdd. Defaults to False
    ...
    Returns:
        ...
        edges (list,optional) : A list of D arrays describing the bin edges for each dimension.
    ...
    if hist_mode == 'numba':
        hist_partition, edges = numba_histogramdd(vals, bins=nBins, ranges=binRanges)
    elif hist_mode == 'numpy':
        hist_partition, edges = np.histogramdd(vals, bins=nBins, range=binRanges)
    else:
        raise ValueError(f'No binning method {hist_mode} available. Please choose between numba and numpy.')
...
def bin_dataframe(
    ...
    **kwds) -> xr.DataArray:
    ...
    Args:
        ...
        jitterParams (dict, optional): Not yet Implemented. Defaults to None.
        pbar (bool, optional): Allows to deactivate the tqdm progress bar. Defaults to True.
        nCores (int, optional): Number of CPU cores to use for parallelization. Defaults to N_CPU-1.
        nThreadsPerWorker (int, optional): Limit the number of threads that multiprocessing can spawn. Defaults to 4.
        threadpoolAPI (str, optional): The API to use for multiprocessing. Defaults to 'blas'.
    ...
    Returns:
        xr.DataArray: The result of the n-dimensional binning represented in an xarray object, combining
            the data with the axes.
    """    
    if jitterParams is not None:
        raise Warning('Jittering is not yet implemented.')
    ...
    if isinstance(binAxes,str):
        ...
        raise ValueError('nBins must be integer or a list of integers for each dimension in axes.')
    ...
    with threadpool_limits(limits=nThreadsPerWorker, user_api=threadpoolAPI):
        ...
        for i in tqdm(range(0, df.npartitions, nCores), disable=not(pbar)):
            ...
            for j in range(0, nCores):
                ...
                coreTasks.append(dask.delayed(bin_partition)(dfPartition, binAxes, nBins, binRanges, hist_mode))
            ...
            if len(coreTasks) > 0:
                ...
                if mode == 'legacy':
                    ...
                    for j in range(0, nCores):
                        ...
                        combineTasks.append(dask.delayed(reduce)(_arraysum, combineParts))
...
def applyJitter(df: Union[dask.dataframe.core.DataFrame,pd.DataFrame], amp:float, col:str, mode:str='uniform'):
    ...
    Args:
        ...
        mode (str, optional): Choose between 'uniform' for uniformly distributed noise, 
            or 'normal' for noise with normal distribution. For columns with digital values, 
            one should choose 'uniform' as well as amplitude (amp) equal to half the step size.
            Defaults to 'uniform'.
    """
...
def numba_histogramdd(sample:np.array, bins:Sequence, ranges:Sequence) -> Tuple[np.array,np.array]:
    """ Wrapper for the Number pre-compiled binning functions. 

    Behaves in total much like numpy.histogramdd. Returns uint32 arrays.
    This was chosen because it has a significant performance improvement over uint64
    for large binning volumes. Be aware that this can cause overflows for very large 
    sample sets exceeding 3E9 counts in a single bin. This should never happen in a 
    realistic photoemission experiment with useful bin sizes.   
    ...
    Raises:
        ...
        NotImplementedError: When attempting binning in too high number of dimensions (>4)
        RuntimeError: Internal shape error after binning

    Returns:
        hist (np.array): The computed histogram
        edges (np.array): A list of D arrays describing the bin edges for each dimension.
    """      
