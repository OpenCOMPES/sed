...
class MetaHandler():
    ...
    def add(self,v: Dict, duplicate: bool='raise') -> None:
        ...
        Args:
            ...
            overwrite (str, optional): Control behaviour in case the 'name' key is
                already present in the metadata dictionary. If raise, rieses a DuplicateEntryError.
                If 'overwrite' it overwrites the previous data with the new one.
        ...
        d = {
            ...
            # 'timezone':datetime.timetz(),
            }
        
        if v['name'] not in self._m.keys() or duplicate == 'overwrite':
            self._m[v['name']] = v
        elif duplicate == 'raise':
            raise DuplicateEntryError(f"an entry {v['name']} already exists in metadata")
            ...
            raise ValueError(f'could not interpret duplication handling method {duplicate}. Please choose between overwrite,append or rise.')
